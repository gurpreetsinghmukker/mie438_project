...
void MPU6050_Initialization(void)
{
	HAL_Delay(50);
	uint8_t who_am_i = 0;
	printf_("Checking MPU6050...\n");

	MPU6050_Readbyte(MPU6050_WHO_AM_I, &who_am_i);
	while(who_am_i != 0x68){
		if(who_am_i == 0x68){
			printf_("MPU6050 who_am_i = 0x%02x...OK\n", who_am_i);
			break;
		}
			printf_("who am i error. Can not recognize mpu6050\n");
			HAL_Delay(100);
			MPU6050_Readbyte(MPU6050_WHO_AM_I, &who_am_i);
	}
	...
	printf_("LSB_Sensitivity_GYRO: %f, LSB_Sensitivity_ACC: %f\n",LSB_Sensitivity_GYRO, LSB_Sensitivity_ACC);
	...
	printf_("MPU6050 setting is finished\n");
}
...
{
	//printf_("LSB_Sensitivity_GYRO: %f, LSB_Sensitivity_ACC: %f\n",LSB_Sensitivity_GYRO,LSB_Sensitivity_ACC);
	mpu6050->acc[0] = mpu6050->acc_x_raw / LSB_Sensitivity_ACC;
	mpu6050->acc[1] = mpu6050->acc_y_raw / LSB_Sensitivity_ACC;
	mpu6050->acc[2] = mpu6050->acc_z_raw / LSB_Sensitivity_ACC;

	mpu6050->temperature = (float)(mpu6050->temperature_raw)/340+36.53;

	mpu6050->gyro[0] = mpu6050->gyro_x_raw / LSB_Sensitivity_GYRO;
	mpu6050->gyro[1] = mpu6050->gyro_y_raw / LSB_Sensitivity_GYRO;
	mpu6050->gyro[2] = mpu6050->gyro_z_raw / LSB_Sensitivity_GYRO;
}
